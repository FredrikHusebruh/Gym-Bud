...
export default function ActiveWorkout({ workout, onClose }: Props) {
    ...
    async function handleStart() {
        setStarting(true)
        try {
            const res = await startWorkout(workout.workoutId)
    ...
    return (
        <div className="w-full flex flex-col bg-gradient-to-br from-[#2a1fff] to-[#1a0aaa]">
            ...
            <div className="w-full h-auto p-4 pb-32 flex flex-col gap-3 bg-black">
                {workoutId
                    ? <SessionWorkout workoutId={workoutId} templateId={workout.workoutId} exercises={workout.exercises} />
                    : workout.exercises.map(e => (
                        <div key={e.exerciseId} className="w-full rounded-3xl p-4 border border-border bg-surface flex flex-col gap-1">
                            <p className="text-text-dim text-start font-extrabold text-lg uppercase">{e.exerciseName}</p>
                            <p className="text-text font-extrabold rounded-full bg-surface-2 px-3 py-1 text-sm uppercase w-fit">{e.muscleGroupName}</p>
                        </div>
                    ))
                }
            </div>
            <div className="fixed w-full bottom-0 left-0 right-0 flex justify-center px-4 pb-8">
                {workoutId
                    ...
                    : <button onClick={handleStart} disabled={starting} className="flex items-center justify-center font-extrabold italic text-xl text-text-dark w-full bg-accent gap-2 rounded-full p-4 disabled:opacity-50">
                        <PlayIcon className="w-8 h-8" />{starting ? 'STARTING...' : 'START WORKOUT'}
                      </button>
                }
            </div>
        </div>
